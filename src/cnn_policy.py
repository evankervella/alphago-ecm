...
class CNN :
    ...
    """
    def __init__(self, layers: list=None, name: str=None) -> None:
    ...
    @staticmethod
    def create_CNN(size: tuple, layers: int, features_nb: int) -> keras.Sequential:
        CNN = Sequential()
        CNN.add (Conv2D(
        filters = 128,
        kernel_size = (5,5),
    ...
    def load(self, filename) -> keras.Sequential:
        CNN = load_model(filename, custom_objects={'Biais': Biais})
        self.model = CNN

    def pred(self, tensor: list):
