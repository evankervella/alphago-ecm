...
def stone_color_feature(state: object) -> list:
...
def turns_since_move(state: object, maximum: int=8) -> list:
...
def get_liberties(state: object, maximum: int=8) -> list:
    planes = np.zeros((maximum, state.size, state.size))
    for i in range(maximum):
...
def get_capture_size(state: object, maximum: int=8) -> list:
...
def get_atari_size(state: object, maximum: int=8) -> list:
...
def get_liberties_after(state: object, maximum: int=8) -> list:
...
def sensibleness(state: object) -> list:
...
class Preprocess(object):
    ...
    def __init__(self, feature_list: list=DEFAULT_FEATURES) -> None:
    ...
    def state_to_tensor(self, state: object) -> list:
        """Converts a GameState to a Theano-compatible tensor
        """
