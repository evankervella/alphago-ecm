...
class Tools:
    ...
    @staticmethod
    def one_hot_action(action, size: int=19) -> np.array:
    ...
    @staticmethod
    def prepare_data(state_dataset: pd.DataFrame, action_dataset: pd.DataFrame) -> tuple:
    ...
    @staticmethod
    def batch_generator(
        state_dataset: pd.DataFrame,
        action_dataset: pd.DataFrame,
        indices: list,
        batch_size: int,
        transforms: list=[]
        ) -> None:
