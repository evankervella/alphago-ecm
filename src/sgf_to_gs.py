...
"""This script converts a .sgf into a GameState object,
from which we'll be able to extract features for latter learning phase
"""

# .sgf files use 'ab' rather than '(1, 2)' so we'll convert it
# Warning: x relates to columns and y to rows

LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'

def _parse_sgf_move(node_value: str) -> tuple:
    """Given a well-formed move string, returns either PASS_MOVE or the (x,y) position
    """
...
def save_gamestate_to_sgf(
    gamestate: object, 
    path: str, 
    filename: str, 
    black_player_name: str='Unknown',
    white_player_name: str='Unknown', 
    size: int=19, 
    komi: float=7.5
    ) -> None:
